#ifndef _DISORDER_AABB_H_
#define _DISORDER_AABB_H_
	...
	*/
	...
	public:
		...
		*/
		...
		inline AABB() : mMinimum(Vector3::ZERO), mMaximum(Vector3::UNIT_SCALE), mCorners(0)
		{
		...
		inline AABB(Extent e) : mMinimum(Vector3::ZERO), mMaximum(Vector3::UNIT_SCALE), mCorners(0)
		{
			setMinimum( -0.5, -0.5, -0.5 );
			setMaximum( 0.5, 0.5, 0.5 );
			mExtent = e;
		...
		inline AABB(const AABB & rkBox) : mMinimum(Vector3::ZERO), mMaximum(Vector3::UNIT_SCALE), mCorners(0)
		...
		inline AABB( const Vector3& min, const Vector3& max ) : mMinimum(Vector3::ZERO), mMaximum(Vector3::UNIT_SCALE), mCorners(0)
		{
			setExtents( min, max );
		}
		...
		inline AABB(
			...
			float Mx, float My, float Mz ) : mMinimum(Vector3::ZERO), mMaximum(Vector3::UNIT_SCALE), mCorners(0)
		{
		...
		*/
		...
		inline void setExtents(
			...
			float Mx, float My, float Mz )
		{
            BOOST_ASSERT( (mx <= Mx && my <= My && mz <= Mz) &&
                "The minimum corner of the box must be less than or equal to maximum corner" );
		...
		*/
		...
		{
			BOOST_ASSERT( (mExtent == EXTENT_FINITE) && "Can't get corners of a null or infinite AAB" );
		...
		*/
		void transformAffine(const Matrix4& m)
		{
			...
			Vector3 newHalfSize(
				Abs<float>(m[0][0]) * halfSize.x + Abs<float>(m[0][1]) * halfSize.y + Abs<float>(m[0][2]) * halfSize.z, 
				Abs<float>(m[1][0]) * halfSize.x + Abs<float>(m[1][1]) * halfSize.y + Abs<float>(m[1][2]) * halfSize.z,
				Abs<float>(m[2][0]) * halfSize.x + Abs<float>(m[2][1]) * halfSize.y + Abs<float>(m[2][2]) * halfSize.z);
		...
		*/
		...
		{
			BOOST_ASSERT( (mExtent == EXTENT_FINITE) && "Can't get center of a null or infinite AAB" );
        ...
        */
		...
		float distance(const Vector3& v) const
		{
			...
			else
			{
				...
				if (v.x < mMinimum.x)
					maxDist = Max<float>(maxDist, mMinimum.x - v.x);
				if (v.y < mMinimum.y)
					maxDist = Max<float>(maxDist, mMinimum.y - v.y);
				if (v.z < mMinimum.z)
					maxDist = Max<float>(maxDist, mMinimum.z - v.z);
				
				if (v.x > mMaximum.x)
					maxDist = Max<float>(maxDist, v.x - mMaximum.x);
				if (v.y > mMaximum.y)
					maxDist = Max<float>(maxDist, v.y - mMaximum.y);
				if (v.z > mMaximum.z)
					maxDist = Max<float>(maxDist, v.z - mMaximum.z);
