...
namespace Disorder
{
	...
	{
		typedef boost::unordered_map<std::string,RenderTechniquePtr> TechniqueMap;
		typedef boost::unordered_map<std::string,RenderLayoutPtr>  RenderLayoutMap;
      
	public:

		virtual RenderTechniquePtr CreateRenderTechnique(std::string const& fileName, ShaderModel shaderModel,std::string const& entryPointVS,std::string const& entryPointPS) = 0;
		virtual RenderLayoutPtr CreateRenderLayout(VertexShaderPtr const& vertexShader,const VertexInputDes* pVertexInputDes,unsigned int vertexInputDesLength,TopologyType topologyType) = 0;
