...
namespace Disorder
{
	...
	void Canvas::DrawString(float xPos,float yPos, float size, Eigen::Vector4f const& color,std::string const& str)
	{
		...
		for(unsigned int index=0;index<str.length();index++)
		{
			...
			pVertexBuffer[0].position << xTemp,yTemp,0.f;
			pVertexBuffer[0].color = color;
			pVertexBuffer[0].texcoord << rect.uvRect.left,rect.uvRect.top;

			pVertexBuffer[1].position << xTemp+drawSizeX,yTemp,0.f;
			pVertexBuffer[1].color = color;
			pVertexBuffer[1].texcoord << rect.uvRect.right,rect.uvRect.top;

			pVertexBuffer[2].position << xTemp,yTemp-size,0.f;
			pVertexBuffer[2].color = color;
			pVertexBuffer[2].texcoord << rect.uvRect.left,rect.uvRect.bottom;

			pVertexBuffer[3].position << xTemp+drawSizeX,yTemp - size,0.f;
			pVertexBuffer[3].color = color;
			pVertexBuffer[3].texcoord << rect.uvRect.right,rect.uvRect.bottom;
